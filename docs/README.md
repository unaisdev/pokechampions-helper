# Technical documentation

Index of engineering docs for **PokeChampions Helper**. Product copy in the app may stay locale-specific until i18n is added; **this folder is maintained in English**.

| Area               | Document                                                               |
| ------------------ | ---------------------------------------------------------------------- |
| Architecture       | [architecture/overview.md](architecture/overview.md)                   |
| Decisions (ADRs)   | [adr/README.md](adr/README.md)                                         |
| PokéAPI usage      | [api/pokeapi.md](api/pokeapi.md)                                       |
| On-device ML / OCR | [ai/model.md](ai/model.md)                                             |
| Setup              | [guides/setup.md](guides/setup.md)                                     |
| Local development  | [guides/local-development.md](guides/local-development.md)             |
| Releases           | [guides/release-process.md](guides/release-process.md)                 |
| Coding style       | [contributing/coding-guidelines.md](contributing/coding-guidelines.md) |
| Testing            | [testing-strategy.md](testing-strategy.md)                             |
| Performance        | [performance-guidelines.md](performance-guidelines.md)                 |
| Design / UI scale  | [design/metrics.md](design/metrics.md)                                 |

Repository-level contribution flow: [CONTRIBUTING.md](../CONTRIBUTING.md).
